...
use crate::HyperCraftHalTrait;
use crate::arch::hvc::run_guest_by_trap2el2;

/// (v)CPU register state that must be saved or restored when 
/// entering/exiting a VM or switching between VMs.
#[repr(C)] #[derive(Clone, Debug)] pub struct VmCpuRegisters {

    pub guest_trap_context_regs: ContextFrame, /// guest trap context
    pub save_for_os_context_regs: ContextFrame, /// arceos context
...
#[derive(Clone)] pub struct VCpu<H:HyperCraftHalTrait> {
    /// Vcpu id
    pub vcpu_id: usize,
    /// Vcpu context
...
impl <H:HyperCraftHalTrait> VCpu<H> {
    ...
    fn init_vm_context(&mut self) {
        ...
        self.regs.vm_system_regs.vtcr_el2 = (
            ...
            + VTCR_EL2::TG0::Granule4KB + VTCR_EL2::SH0::Inner
            + VTCR_EL2::ORGN0::NormalWBRAWA + VTCR_EL2::IRGN0::NormalWBRAWA
            + VTCR_EL2::SL0.val(0b01) + VTCR_EL2::T0SZ.val(64 - 40)
        ).into();
        ...
        let mut vmpidr = 0; vmpidr |= 1 << 31; vmpidr |= self.vcpu_id;
    ...
    fn vcpu_arch_init(&mut self, kernel_entry_point: usize, device_tree_ipa: usize) {
        ...
        self.regs.guest_trap_context_regs.spsr =( 
            SPSR_EL1::M::EL1h + SPSR_EL1::I::Masked + SPSR_EL1::F::Masked + 
            SPSR_EL1::A::Masked + SPSR_EL1::D::Masked
        ).value;
    }
